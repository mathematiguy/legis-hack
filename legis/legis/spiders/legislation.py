...
class LegislationSpider(scrapy.Spider):
    ...
    def __init__(self, from_year = 1993, to_year = 1993, **kwargs):
    ...
    def parse(self, response):
        ...
        for li in li_tags:
            ...
            file_regex   = re.compile("/subscribe/act/public/(.*)")
            folder_regex = re.compile("/subscribe/act/public/(.*)/[^/]+$")
            if li_class == "directory":
                ...
                folder_path = os.path.join(
                    self.root_dir, 
                    file_regex.search(href).group(1))
            	...
            	if not href.endswith(".xml"):
            		self.logger.info("class %s found is not file or directory for tag %s" %(li_class, li.extract()))
